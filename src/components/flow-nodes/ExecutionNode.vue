  ...
  <div :class="['custom-node', 'execution-node', { selected }]">
    ...
    <div class="node-content">
      <div class="execution-info">
        <div v-if="data.nodeType === 'execution-dowhile'" class="info-text">循环执行直到条件满足</div>
...
defineEmits(['run'])
function getIcon() { return props.data.nodeType === 'execution-dowhile' ? '🔁' : '↻' }
</script>

<style scoped>
.custom-node {
  min-width: 120px; width: 100%; background: white; border: 2px solid #06b6d4;
  border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,.1);
}
.custom-node.selected { border-color: #0891b2; box-shadow: 0 4px 16px rgba(8,145,178,.3); }
.node-header {
  display: flex; align-items: center; padding: 10px 12px;
  background: #06b6d4; color: white; font-weight: 600; border-radius: 6px 6px 0 0;
}
...
.execution-info { text-align: center; }
