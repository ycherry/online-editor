<template>
  <div :class="['etl-node', nodeTypeCls, { selected }]">
    <Handle v-if="hasTarget" type="target" :position="Position.Left" id="a"
      :style="isMultiInput ? { top: '33%' } : {}" />
    <Handle v-if="isMultiInput" type="target" :position="Position.Left" id="b"
      style="top: 67%" />
...
.etl-node:hover {
  border-color: #4a90e2;
  box-shadow: 0 2px 8px rgba(74, 144, 226, .2);
}
...
.dot-running { background: #f59e0b; animation: pulse .8s infinite; }
