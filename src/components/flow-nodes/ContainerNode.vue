  ...
  <div :class="['custom-node', 'container-node', { selected }]">
    ...
    <div class="node-content">
      <div v-if="data.nodeType === 'container-list'" class="container-info">
        <div class="info-item"><span class="label">元素数:</span><span class="value">{{ data.elements?.length || 0 }}</span></div>
      </div>
      <div v-else class="container-info">
        <div class="info-item"><span class="label">键值对:</span><span class="value">{{ data.pairs?.length || 0 }}</span></div>
      </div>
    </div>
  </div>
</template>
...
defineEmits(['run'])
function getIcon() { return props.data.nodeType === 'container-list' ? '📋' : '📖' }
</script>

<style scoped>
.custom-node {
  min-width: 120px; width: 100%; background: white; border: 2px solid #ec4899;
  border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,.1);
}
.custom-node.selected { border-color: #db2777; box-shadow: 0 4px 16px rgba(219,39,119,.3); }
.node-header {
  display: flex; align-items: center; padding: 10px 12px;
  background: #ec4899; color: white; font-weight: 600; border-radius: 6px 6px 0 0;
}
...
.container-info { display: flex; flex-direction: column; gap: 4px; }
