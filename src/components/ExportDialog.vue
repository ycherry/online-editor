        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            ...
            <div class="format-btns">
              <button :class="['fmt-btn', format === 'csv' && 'active']" @click="format = 'csv'">CSV</button>
              <button :class="['fmt-btn', format === 'json' && 'active']" @click="format = 'json'">JSON</button>
            </div>
          </div>
          <div class="form-row">
            <label>选择节点</label>
          ...
          <div v-if="selectedNodes.length > 0" class="preview-hint">已选 {{ selectedNodes.length }} 个节点</div>
        </div>
        <div class="modal-footer">
          <button class="btn" @click="$emit('close')">取消</button>
          <button class="btn primary" :disabled="selectedNodes.length === 0" @click="handleExport">导出</button>
        </div>
      </div>
    </div>
  </Teleport>
...
const exportableNodes = computed(() => (props.nodes || []).filter(n => {
  const id = n.id
...
.input { width: 100%; padding: 8px 10px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 13px; box-sizing: border-box; }
.format-btns { display: flex; gap: 6px; }
.fmt-btn { padding: 6px 16px; border: 1px solid #d1d5db; border-radius: 6px; cursor: pointer; background: white; font-size: 13px; }
.fmt-btn.active { background: #4a90e2; border-color: #4a90e2; color: white; }
.node-list { border: 1px solid #e5e7eb; border-radius: 6px; max-height: 180px; overflow-y: auto; padding: 6px; }
.node-check { display: flex; align-items: center; gap: 8px; padding: 4px 6px; cursor: pointer; border-radius: 4px; font-size: 13px; }
.node-check:hover { background: #f9fafb; }
.preview-hint { font-size: 12px; color: #6b7280; }
