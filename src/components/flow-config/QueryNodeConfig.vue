  ...
  <div class="config-form">
    ...
    <template v-else-if="localData.nodeType === 'query-api'">
      ...
      <div class="mappings-list">
        <div v-for="(m, i) in (localData.fieldMappings || [])" :key="i" class="mapping-row">
          <input v-model="m.inputField" class="map-input" placeholder="输入字段名（如：三级专业）" @input="emitUpdate" />
          <span class="arrow">→</span>
          <input v-model="m.apiParam" class="map-input" placeholder="API参数名（如：thirdCategory）" @input="emitUpdate" />
    ...
    <template v-else-if="localData.nodeType === 'query-field'">
      ...
      <div class="form-group">
        ...
        <input v-model="localData.extractField" type="text" class="form-input"
          placeholder="如：综合单价" @input="emitUpdate" />
      </div>
      <div class="form-group">
...
watch(() => props.modelValue, (v) => {
  localData.value = {
    matchMode: 'exact', fieldMappings: [], extractFirst: false,
    ...v
  }
}, { deep: true })
...
.section-title { font-size: 13px; font-weight: 700; color: #1f2937; padding-bottom: 4px; border-bottom: 1px solid #e5e7eb; }
...
.info-box { padding: 8px 12px; background: #f0fdfa; border-left: 3px solid #14b8a6; border-radius: 4px; font-size: 12px; color: #134e4a; line-height: 1.5; }
.mappings-list { display: flex; flex-direction: column; gap: 8px; }
.mapping-row { display: flex; align-items: center; gap: 6px; }
.map-input { flex: 1; padding: 6px 10px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 12px; }
.map-input:focus { outline: none; border-color: #14b8a6; }
.arrow { color: #6b7280; font-size: 14px; flex-shrink: 0; }
.btn-remove-map { background: none; border: none; color: #ef4444; cursor: pointer; font-size: 16px; padding: 0 4px; flex-shrink: 0; }
.btn-add-map { align-self: flex-start; background: #f0fdfa; border: 1px dashed #14b8a6; color: #0d9488; border-radius: 6px; padding: 5px 12px; font-size: 12px; cursor: pointer; }
.btn-add-map:hover { background: #ccfbf1; }
</style>
