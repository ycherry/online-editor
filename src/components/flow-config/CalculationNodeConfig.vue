    ...
    <div class="form-group">
      ...
      <div class="params-list">
        <div v-for="(param, idx) in localData.params" :key="idx" class="param-item">
          ...
          <input v-model="localData.params[idx]" type="text" class="form-input" placeholder="参数名称或数值" @input="emitUpdate" />
...
if (!localData.value.params || localData.value.params.length === 0) localData.value.params = ['', '']
...
function removeParam(idx) { localData.value.params.splice(idx, 1); emitUpdate() }
...
.form-input, .form-select { padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 13px; }
...
.param-index { width: 24px; height: 24px; background: #8b5cf6; color: white; border-radius: 50%; font-size: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.form-input { flex: 1; }
.btn-remove { padding: 4px 8px; background: #fee2e2; color: #dc2626; border: 1px solid #fecaca; border-radius: 4px; cursor: pointer; font-size: 14px; }
...
.btn-add:hover { border-color: #8b5cf6; color: #8b5cf6; background: #f5f3ff; }
.expression-preview { padding: 10px 16px; background: #f3f4f6; border-radius: 6px; font-family: monospace; font-size: 13px; color: #1f2937; }
</style>
