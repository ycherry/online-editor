  ...
  <div class="config-form">
    <div class="form-group">
      ...
      <div class="operator-badge">
        {{
          localData.nodeType === 'logic-and'
            ? 'AND (且)'
            : localData.nodeType === 'logic-or'
              ? 'OR (或)'
              : 'NOR (都不)'
        }}
      </div>
      ...
      <div class="params-list">
        <div v-for="(param, idx) in localData.params" :key="idx" class="param-item">
          ...
          <input
            ...
            type="text"
            class="form-input"
            placeholder="参数名称（连接其他节点）"
            @input="emitUpdate"
          />
          <button class="btn-remove" @click="removeParam(idx)">×</button>
        </div>
        <button v-if="localData.nodeType !== 'logic-nor'" class="btn-add" @click="addParam">
          + 添加参数
        </button>
      </div>
    </div>
...
<script setup>
  ...
  function removeParam(idx) {
    if (localData.value.params.length > 2) {
