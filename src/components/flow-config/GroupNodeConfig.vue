  ...
  <div class="config-form">
    <div class="form-group">
      ...
      <div v-for="(g, i) in cfg.groupFields" :key="i" class="tag-row">
        <input v-model="cfg.groupFields[i]" class="form-input" placeholder="字段名"
          @input="emit('update:modelValue', { ...cfg })" />
        <button class="del-btn" @click="cfg.groupFields.splice(i,1); emit('update:modelValue', {...cfg})">✕</button>
      </div>
      <button class="add-btn" @click="cfg.groupFields.push(''); emit('update:modelValue', {...cfg})">+ 添加分组字段</button>
    </div>
    <div class="form-group">
      <label class="form-label">汇总计算</label>
      <div v-for="(agg, i) in cfg.aggregations" :key="i" class="agg-row">
        <input v-model="agg.field" class="form-input" placeholder="字段名"
          @input="emit('update:modelValue', { ...cfg })" />
        <select v-model="agg.func" class="func-select"
        ...
        <input v-model="agg.alias" class="form-input" placeholder="输出字段名（别名）"
          @input="emit('update:modelValue', { ...cfg })" />
        <button class="del-btn"
          @click="cfg.aggregations.splice(i,1); emit('update:modelValue', {...cfg})">✕</button>
      </div>
...
.form-input { flex: 1; padding: 5px 8px; border: 1px solid #d1d5db; border-radius: 5px; font-size: 12px; min-width: 0; }
.form-input:focus { outline: none; border-color: #4a90e2; }
.tag-row, .agg-row { display: flex; align-items: center; gap: 6px; }
.func-select { padding: 5px 6px; border: 1px solid #d1d5db; border-radius: 5px; font-size: 12px; width: 76px; flex-shrink: 0; }
