  ...
  <div class="config-form">
    ...
    <template v-if="localData.nodeType === 'condition-compare'">
      ...
      <div class="expression-preview">
        {{ localData.left || '左值' }} {{ localData.operator }} {{ localData.right || '右值' }}
      </div>
    </template>
    <!-- 集合判断节点 -->
    <template v-else-if="localData.nodeType === 'condition-belongs'">
      ...
      <div class="form-group">
        ...
        <textarea
          v-model="localData.container"
          class="form-textarea"
          rows="3"
          placeholder="[1, 2, 3] 或连接列表节点"
          @input="emitUpdate"
        ></textarea>
      </div>
    </template>
