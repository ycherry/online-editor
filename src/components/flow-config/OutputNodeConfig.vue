<template>
  <div class="config-form">
    <div class="form-group">
      ...
      <input v-model="localData.filename" type="text" class="form-input" placeholder="输出数据" @input="emitUpdate" />
    </div>
    ...
    <div v-else class="info-box">
      ...
      <em>接线说明：</em>输入1 = 材料清单表格数据；输入2（可选）= 单价标量（来自字段提取或运算节点），
      若接入则用于计算 合价 = 数量 × 单价。
    </div>
    ...
    <div class="export-section">
      ...
      <div v-else class="result-info pending">
        ⏳ 请先执行工作流，再导出 Excel
      </div>
      <button
        class="export-btn"
...
.info-box {
  padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #22c55e;
  border-radius: 4px; font-size: 12px; color: #166534;
}

.columns-list { display: flex; flex-wrap: wrap; gap: 6px; }
.col-tag {
  padding: 3px 9px; background: #dcfce7; border: 1px solid #86efac;
  border-radius: 12px; font-size: 11px; color: #166534; font-weight: 500;
}
...
.result-info { font-size: 12px; color: #374151; }
.result-info.pending { color: #9ca3af; }
.export-btn {
  padding: 9px 16px; background: #22c55e; border: none; border-radius: 6px;
  color: white; font-size: 13px; font-weight: 600; cursor: pointer; transition: background .15s;
}
