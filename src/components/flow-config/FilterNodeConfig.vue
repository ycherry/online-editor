  ...
  <div class="config-form">
    <div class="form-group">
      ...
      <div v-for="(cond, i) in cfg.conditions" :key="i" class="cond-row">
        <select v-model="cond.logic" v-if="i > 0" class="logic-select"
          @change="emit('update:modelValue', { ...cfg })">
          <option value="and">AND</option>
          <option value="or">OR</option>
        </select>
        <span v-else class="logic-placeholder">WHERE</span>
        <input v-model="cond.field" class="form-input" placeholder="字段名"
        ...
        <input v-model="cond.value" class="form-input" placeholder="值"
          @input="emit('update:modelValue', { ...cfg })" />
        <button class="del-btn"
          @click="cfg.conditions.splice(i,1); emit('update:modelValue', {...cfg})">✕</button>
      </div>
...
.logic-select { width: 64px; flex-shrink: 0; padding: 5px 4px; border: 1px solid #d1d5db; border-radius: 5px; font-size: 12px; }
.logic-placeholder { width: 64px; flex-shrink: 0; font-size: 12px; font-weight: 600; color: #6b7280; text-align: center; }
.op-select { width: 72px; flex-shrink: 0; padding: 5px 4px; border: 1px solid #d1d5db; border-radius: 5px; font-size: 12px; }
