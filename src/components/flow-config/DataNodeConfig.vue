  ...
  <div class="config-form">
    <div class="form-group">
      ...
      <div class="file-upload-area">
        <input ref="fileInputRef" type="file" accept=".csv,.json" style="display:none" @change="handleFileSelect" />
        ...
        <div v-else class="file-selected">
          <div class="file-info-row">
            ...
            <div class="file-details">
              ...
              <div class="file-meta">{{ localData.totalRows || 0 }} 行 × {{ localData.totalCols || 0 }} 列</div>
            </div>
    ...
    <div v-if="previewData.length > 0" class="form-group">
      ...
      <div class="preview-table-container">
        <table class="preview-table">
          <thead><tr><th v-for="(h, i) in previewData[0]" :key="i">{{ h }}</th></tr></thead>
          <tbody>
...
.form-input:focus { outline: none; border-color: #4a90e2; box-shadow: 0 0 0 2px rgba(74,144,226,.15); }
.upload-prompt {
  border: 2px dashed #d1d5db; border-radius: 8px; padding: 24px;
  text-align: center; cursor: pointer; transition: all .2s;
}
.upload-prompt:hover { border-color: #4a90e2; background: #f0f7ff; }
.upload-icon { font-size: 28px; margin-bottom: 8px; }
.upload-prompt p { margin: 4px 0; font-size: 13px; color: #555; }
.upload-prompt .hint { font-size: 11px; color: #999; }
.file-selected { border: 1px solid #d1d5db; border-radius: 8px; padding: 12px; background: #f9fafb; }
.file-info-row { display: flex; align-items: center; gap: 10px; }
.file-icon { font-size: 24px; }
.file-details { flex: 1; }
.file-name { font-size: 13px; font-weight: 600; color: #333; }
.file-meta { font-size: 11px; color: #666; margin-top: 2px; }
.btn-change { padding: 4px 10px; background: #4a90e2; color: white; border: none; border-radius: 4px; font-size: 12px; cursor: pointer; }
.btn-change:hover { background: #357abd; }
.error-text { font-size: 12px; color: #ef4444; margin: 0; }
.preview-table-container { overflow-x: auto; max-height: 200px; overflow-y: auto; }
.preview-table { width: 100%; border-collapse: collapse; font-size: 11px; }
.preview-table th, .preview-table td { padding: 4px 8px; border: 1px solid #e5e7eb; white-space: nowrap; }
.preview-table th { background: #f3f4f6; font-weight: 600; position: sticky; top: 0; }
.info-box { padding: 12px 16px; background: #dbeafe; border-left: 4px solid #3b82f6; border-radius: 4px; font-size: 13px; color: #1e40af; }
.info-box p { margin: 0; }
</style>
