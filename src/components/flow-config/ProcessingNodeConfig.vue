  ...
  <div class="config-form">
    ...
    <template v-if="localData.nodeType === 'processing-extreme'">
      ...
      <div class="form-group">
        ...
        <input v-model="localData.list" type="text" class="form-input" placeholder="连接数据源或输入列表" @input="emitUpdate" />
      </div>
    </template>
    <!-- 平均值 -->
    <template v-else-if="localData.nodeType === 'processing-average'">
      ...
      <div class="form-group">
        ...
        <input v-model="localData.values" type="text" class="form-input" placeholder="连接数据源" @input="emitUpdate" />
      </div>
      <div v-if="localData.method === 'weighted'" class="form-group">
        <label class="form-label">权重列表</label>
        <input v-model="localData.weights" type="text" class="form-input" placeholder="输入权重数据" @input="emitUpdate" />
      </div>
    </template>
    <!-- 缺失值处理 -->
    ...
    <template v-else-if="localData.nodeType === 'processing-price'">
      <div class="form-group">
        ...
        <input v-model="localData.data" type="text" class="form-input" placeholder="连接原始数据源" @input="emitUpdate" />
      </div>
      <div class="form-group">
        <label class="form-label">大宗材料价格数据</label>
        <input v-model="localData.priceData" type="text" class="form-input" placeholder="连接价格数据源" @input="emitUpdate" />
      </div>
    </template>
  </div>
</template>
...
.info-box { padding: 12px; background: #f0f9ff; border-left: 3px solid #6366f1; border-radius: 4px; font-size: 12px; color: #1e40af; }
.info-box p { margin: 0; }
</style>
