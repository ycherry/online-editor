  ...
  <div class="config-form">
    <div class="form-group">
      ...
      <div v-for="(f, i) in cfg.fields" :key="i" class="field-row">
        <input v-model="f.source" class="form-input" placeholder="原字段名"
          @input="emit('update:modelValue', { ...cfg })" />
        <input v-model="f.alias" class="form-input" placeholder="别名（留空保持原名）"
          @input="emit('update:modelValue', { ...cfg })" />
        <div style="width:60px;text-align:center">
          <input type="checkbox" v-model="f.keep"
...
.field-header { display: flex; align-items: center; gap: 6px; padding: 0 2px; margin-bottom: 4px; }
.col-title { font-size: 11px; font-weight: 600; color: #6b7280; }
.field-row { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; }
.form-input { flex: 1.2; padding: 5px 8px; border: 1px solid #d1d5db; border-radius: 5px; font-size: 12px; min-width: 0; }
.form-input:focus { outline: none; border-color: #4a90e2; }
.del-btn { width: 28px; border: none; background: none; cursor: pointer; color: #9ca3af; font-size: 13px; padding: 2px; border-radius: 3px; }
