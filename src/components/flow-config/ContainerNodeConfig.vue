      ...
      <div class="form-group">
        ...
        <div class="elements-list">
          <div v-for="(element, idx) in localData.elements" :key="idx" class="element-item">
            ...
            <input
              v-model="localData.elements[idx]"
              type="text"
              class="form-input"
              placeholder="元素值"
              @input="emitUpdate"
...
<script setup>
  ...
  if ('elements' in (localData.value || {}) && !localData.value.elements)
    localData.value.elements = ['']
  if ('pairs' in (localData.value || {}) && !localData.value.pairs)
    localData.value.pairs = [{ key: '', value: '' }]
  watch(
    () => props.modelValue,
    (v) => {
      localData.value = { ...v }
  ...
  function addElement() {
    localData.value.elements.push('')
    emitUpdate()
  }
  function removeElement(idx) {
    if (localData.value.elements.length > 1) {
      localData.value.elements.splice(idx, 1)
      emitUpdate()
    }
  }
  function addPair() {
    localData.value.pairs.push({ key: '', value: '' })
    emitUpdate()
  }
  function removePair(idx) {
    if (localData.value.pairs.length > 1) {
      localData.value.pairs.splice(idx, 1)
      emitUpdate()
    }
  }
</script>
