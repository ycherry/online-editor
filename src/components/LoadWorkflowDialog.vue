      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div v-else class="workflow-list">
            <div v-for="wf in workflows" :key="wf.id" class="workflow-item" @click="handleSelect(wf)">
              ...
              <div class="workflow-actions">
                ...
                <button class="action-btn danger" @click.stop="handleDelete(wf.id)" title="删除">🗑️</button>
              </div>
            </div>
          </div>
        </div>
...
function handleSelect(wf) { emit('select', wf) }
...
.modal-content { background: white; border-radius: 12px; width: 90%; max-width: 680px; max-height: 80vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,.3); }
...
.modal-body { flex: 1; overflow-y: auto; padding: 20px 24px; min-height: 200px; }
...
.workflow-item { display: flex; align-items: center; padding: 14px 16px; border: 1px solid #e5e7eb; border-radius: 8px; cursor: pointer; transition: all .2s; }
.workflow-item:hover { border-color: #4a90e2; background: #f0f7ff; }
.workflow-info { flex: 1; }
.workflow-name { font-size: 15px; font-weight: 600; color: #111827; margin-bottom: 4px; }
.workflow-desc { font-size: 12px; color: #6b7280; margin-bottom: 6px; }
.workflow-meta { display: flex; gap: 12px; font-size: 11px; color: #9ca3af; }
.workflow-actions { display: flex; gap: 6px; }
.action-btn { background: none; border: 1px solid #e5e7eb; border-radius: 6px; padding: 6px 10px; cursor: pointer; font-size: 14px; transition: all .2s; }
.action-btn:hover { background: #f3f4f6; }
.action-btn.danger:hover { background: #fef2f2; border-color: #fecaca; }
.modal-footer { padding: 14px 24px; border-top: 1px solid #e5e7eb; display: flex; justify-content: flex-end; }
.btn { padding: 9px 20px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 14px; cursor: pointer; background: white; }
