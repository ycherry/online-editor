<template>
  <div class="editor-page" @keydown="handleKeyDown" tabindex="0">
    <FlowToolbar @new="handleNew" @save="showSaveDialog = true" @load="showLoadDialog = true"
      @export="showExportDialog = true" @import="handleImport" @run="handleRun" @clear="handleClear" />
    <div class="editor-main">
      <div class="node-library">
        ...
        <div v-for="cat in nodeCategories" :key="cat.name" class="node-category">
          ...
          <div v-if="cat.open" class="category-nodes">
            <div v-for="item in cat.nodes" :key="item.type" class="node-item" draggable="true"
              @dragstart="onDragStart($event, item)" :style="{ borderLeftColor: item.color }">
              <span>{{ item.icon }}</span>
      ...
      <div class="canvas-area" @drop="onDrop" @dragover.prevent>
        <VueFlow v-model:nodes="nodes" v-model:edges="edges" :connection-mode="ConnectionMode.Loose"
          @node-click="onNodeClick" @pane-click="onPaneClick" @node-context-menu="onNodeContextMenu"
          @connect="onConnect">
          <Background pattern-color="#aaa" :gap="16" />
          <Controls />
          <MiniMap />
          <template #node-data="props">
            <DataNode v-bind="props" @run="runSingleNode" />
          </template>
          ...
          <template #node-comparison="props">
            <ComparisonNode v-bind="props" @run="runSingleNode" :max-zoom="4" :min-zoom="0.1"/>
          </template>
          <template #node-output="props">
            <OutputNode v-bind="props" @run="runSingleNode" />
          </template>
        </VueFlow>
        ...
        <div class="panel-body">
          <div v-if="rightTab === `config`" class="config-panel">
            ...
            <template v-else>
              ...
              <component :is="getConfigComponent(selectedNode.data.nodeType)" v-model="selectedNode.data.config"
                :node-id="selectedNode.id"
                @update:model-value="onNodeDataChange" />
            </template>
          </div>
          <NodePreviewPanel v-else :node-id="previewNodeId" :node-name="previewNodeName" :node-type="previewNodeType"
    ...
    <div :class="[`debug-wrapper`, debugOpen && `open`]">
      ...
      <div v-if="debugOpen" class="debug-content">
        <DebugPanel :mode="executionMode" :is-running="executionStore.isRunning" :is-paused="executionStore.isPaused"
          :progress="executionStore.progress" :current-node-id="executionStore.currentNodeId"
          :breakpoints="executionStore.breakpoints" :logs="debugLogs" :nodes="nodes" @run="handleRun" @step="handleStep"
          @continue="handleContinue" @pause="executionStore.pause()" @stop="handleStop" @clear="clearDebug"
          @set-mode="executionMode = $event" @remove-breakpoint="executionStore.removeBreakpoint($event)"
          @clear-breakpoints="executionStore.clearBreakpoints()" />
      </div>
    </div>
    ...
    <LoadWorkflowDialog :show="showLoadDialog" :workflows="workflowStore.workflows" @close="showLoadDialog = false"
      @select="handleLoadWorkflow" @delete="workflowStore.deleteWorkflow($event)" />
    <ExportDialog :show="showExportDialog" :nodes="nodes" :results="executionStore.nodeResults"
  ...
  const nodeCategories = ref([
    ...
    },
    {
      name: "查询", icon: "🔍", open: false, nodes: [
        ...
        { type: "query-api", label: "后端查询", icon: "🌐", color: "#14b8a6", defaultConfig: { fieldMappings: [] } },
        { type: "query-field", label: "字段提取", icon: "📌", color: "#14b8a6", defaultConfig: { extractField: "", extractFirst: false } }
      ]
    },
    {
